---
description: Hunea 主界面、输入框、覆盖层与工具审批的常用快捷键一览
---
...
| `Ctrl + R` | 打开全局消息历史（与 [`/resend`](/guide/fun/menu/resend.html) 同类） |
...
编辑类按键大体贴近 readline（如 `Ctrl + A/E/B/F/K/U/W`、按词移动与删除等），细节见[输入框](/guide/fun/designs/composer.html)。

### 前缀浮窗

| 前缀 | 作用 |
| --- | --- |
| `/`（输入框为空时输入） | [斜杠菜单](/guide/fun/menu/) |
| `@` | 文件 / 图片路径 |
| `$` | [技能](/guide/fun/skills) |
| `#` | 自定义 prompt |
...
完整命令列表见[斜杠菜单前言](/guide/fun/menu/)。

## 会话恢复 `/resume`、消息复制 `/copy`、历史 `/resend`

这类全屏列表的导航方式大体相近（以界面底部提示为准）：
...
更细的分层操作（分支预览、Branch Tree）见 [`/tree`](/guide/fun/menu/tree.html)。
...
详见 [`/sends-back`](/guide/fun/menu/sends-back.html)。
...
- [`/models`](/guide/fun/menu/models.html)：直接输入进行过滤，`Enter` 选中；`U` 刷新模型列表（若供应商支持）  
- [`/prompt`](/guide/fun/menu/prompt.html)：双栏与页签；`Space` 预览单项，`p` 查看组装结果，`?` 查看该面板帮助，`Ctrl + G` 可用外部编辑器编辑正文  
