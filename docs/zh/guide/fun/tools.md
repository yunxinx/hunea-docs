---
description: Hunea 内置工具清单、工作区路径访问边界、需要审批的操作类型，以及审批面板中的确认与拒绝交互方式
---
...
> 工具相关的使用说明（tool guidelines）会参与 [`/prompt`](/guide/fun/menu/prompt) 的提示词组装；上下文占用也可以在 [`/context`](/guide/fun/menu/context) 中看到 `Tool definitions` 一项。本页补充的是运行时能力本身。
...
请求重试、idle timeout 等说明见[config.toml](/guide/start/configuration/config)中的 `[runtime]` 部分。
