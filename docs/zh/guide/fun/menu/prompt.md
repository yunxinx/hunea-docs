...
`/prompt` 在 Hunea 中的设计为查看、预览并调整**下一次新会话**的  `Prompt Assembly`（提示词组装）时使用。

这个也是 Hunea 后续核心功能的一个初始态。后续会继续进行打磨和完善。不过当前已经可以简单使用。

适用于想确认系统提示由哪些来源拼起来、临时开关某段 `instructions`（指令）/ `skill`（技能）/ `tool`（工具）指南、新增自定义 `prompt`（提示词），或者先预览整段最终 `system prompt`（系统提示词）再决定要不要改时使用。输入 `/prompt` 并确认后，会打开一个全屏面板（标题类似 `Prompt Assembly`（提示词组装））。
