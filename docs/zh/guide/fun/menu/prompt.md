...
这是 Hunea 的核心功能之一，目前已经可以正常使用，后续还会继续打磨完善。
...
| `Tools`（工具） | 工具候选 / 指南片段；逐工具启停（见下） |
| `Dynamic`（动态） | 动态环境来源；可分别勾 `Base`（基线）/ `Change`（变化） |

不同页签的表头会略有不同，例如：

- `Skill`（技能）：`Sel`（选择）· `Ord`（顺序）· `Name`（名称）· `Scope`（作用域）
- `Tools`（工具）：`On` · `Guide` · `Ord`（顺序）· `Name`（名称）· `Scope`（作用域）（On/Guide 为双列开关，见下）
- `Custom Prompts`（自定义提示词）：`Sel`（选择）· `Name`（名称）· `Scope`（作用域）
- `Dynamic`（动态）：`Base`（基线）· `Change`（变化）· `Source`（来源）· `Scope`（作用域）

#### `Tools`（工具）页签：On / Guide 双列

`Tools`（工具）页签每行有两个复选框列，可分别控制：

| 列 | 含义 |
| --- | --- |
| `On` | 工具本体是否启用。关闭后该工具不会出现在发送给模型的工具列表中，模型无法调用它 |
| `Guide` | 该工具的使用指南（guidelines）是否注入系统提示词 |

操作方式：`←` / `→` 在 `On` / `Guide` 两列之间切换（当前列高亮），`x` 切换当前列的开关。需注意：当某工具的 `On` 关闭时，它的 `Guide` 列变为不可操作（弱化显示），此时 `x` 为无操作；本身没有 guidelines 的工具，`Guide` 列同样无操作。禁用的工具会从模型请求与会话工具注册表中一起过滤，并持久化（`tool_enablement`）。详见[工具级启用/禁用](/changelog/tool-enablement)。
...
按 `Esc` 关闭 `/prompt` 时，Hunea 会尽量把本次面板里的工作副本提交/落盘，而不是关掉就丢弃你做的改动。
...
- 想让多数改动完整进入下一轮对话，建议关闭 `/prompt` 后，用 `/clear`（别名 `/new`）开一条新会话。
...
`/context` 告诉你预算还剩多少；`/prompt` 告诉你这些预算里的 `system`（系统）侧由哪些来源拼出来、能不能改。
...
- 快捷键记不全时，按 `?` 看 `More`（更多），不用去猜页脚那些缩写。
