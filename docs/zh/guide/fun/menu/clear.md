---
description: 介绍 Hunea 的 /clear 斜杠命令的作用
---

# /clear

`/clear` 在 Hunea 中的设计为清空当前对话上下文、开一条新会话时使用。

`/new` 是它的别名。输入时两者都能匹配到同一项：当你键入 `/new` 时，补全/选中后执行的仍是清空当前对话；键入 `/clear` 也一样。

适用于当前会话已经很长、上下文占满、想换个干净起点继续聊，但还不想退出应用时。在输入框为空时输入 `/`，选中 `/clear`（或通过 `/new` 匹配到它），再按 `Enter` 即可。

> 当前版本**没有二次确认**：选中并 `Enter` 后会立刻执行。后续可能也不会做二次确认机制。

## 执行之后会发生什么

确认 `/clear` 后，Hunea 大致会做两件事：

1. **把当前 TUI 界面恢复成接近刚启动时的状态**
2. **重置当前运行时会话**，让后续请求走一条新的、干净的对话上下文

界面侧常见变化包括：

- 当前会话的可见对话记录被清掉
- 启动横幅（startup banner）会重新出现
- 输入框里的草稿会被清空
- 各类覆盖层/内联面板会尽量关掉（例如模型选择、会话恢复、会话树、复制面板、审批面板、文件/技能选择等）
- 流式输出中的状态、选择区、轻提示等也会一并清掉

运行时侧则更像“重新开一轮会话”：

- 若当时还有进行中的请求，通常会被中断/取消
- 发给供应商的历史上下文会被清掉
- 下一次发送会按**新会话**去准备（包括重新取用“下一次新会话”的提示词装配结果）

## 不会清掉什么

`/clear` 清的是**当前对话上下文**，不是把 Hunea 的所有数据都抹掉。通常不会因为 `/clear` 而丢掉：

- **当前选中的模型**  
  一般会继续沿用，不必重新 `/models` 选一遍（当然你也可以再换）。

- **已经保存过的历史会话**  
  之前聊过并落盘的会话，通常仍可在 [`/resume`](/guide/fun/menu/resume.html) 列表里找回来；`/clear` 更像“切到新的空会话”，不是批量删除存档。

- **全局用户消息历史**  
  [`/resend`](/guide/fun/menu/resend.html) 用的那份跨项目历史，不会因为 `/clear` 被清空。

- **`models.toml`、prompt assembly、配置文件**  
  供应商/模型配置、以及你在 [`/prompt`](/guide/fun/menu/prompt.html) 里维护的装配状态，都不会被 `/clear` 直接改掉。

## 和相邻命令的区别

| 你想做的事 | 更合适的命令 |
| --- | --- |
| 清空当前对话、开一条新会话，但继续留在应用里 | `/clear`（别名 `/new`） |
| 退出整个应用 | [`/exit`](/guide/fun/menu/quit.html)（别名 `/quit`） |
| 回到某条已保存的旧会话 | [`/resume`](/guide/fun/menu/resume.html) |
| 只回填以前发过的用户输入 | [`/resend`](/guide/fun/menu/resend.html) |
| 在当前会话里精确回到某个消息节点 / 开分支 | [`/tree`](/guide/fun/menu/tree.html) |
| 查看上下文大概占了多少 | [`/context`](/guide/fun/menu/context.html) |

简单说：

- **还想继续用 Hunea，只是对话太乱/太满了** → `/clear`
- **已经聊完了，要关掉程序** → `/exit`
- **不是不要当前对话，而是想找回以前某条会话** → `/resume`

## 一点使用建议

- 上下文快满、模型开始“忘事”或答非所问：先看一眼 [`/context`](/guide/fun/menu/context.html)，需要时直接 `/clear` 开干净会话。
- 刚在 [`/prompt`](/guide/fun/menu/prompt.html) 里改过提示词装配：多数改动面向“下一次新会话”；关面板后再 `/clear`，更容易立刻验证新装配是否生效。
- 当前会话里还有想带走的内容：先用 [`/copy`](/guide/fun/menu/copy.html) 拷出去，再 `/clear`。
- 只是想复用某条旧提示词、并不想丢掉当前对话：用 [`/resend`](/guide/fun/menu/resend.html)，不要用 `/clear`。
